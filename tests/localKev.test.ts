...
describe('local-kev parseArgs', () => {
  it('defaults to kev-0.8b on port 8009, in .local/kev', () => {
    expect(parseArgs([])).toEqual({
      ok: true,
      value: { model: 'kev-0.8b', port: 8009, dir: '.local/kev', cuda: false, help: false },
    })
  })
  ...
  it('accepts --model, --port, --dir and --cuda', () => {
    expect(parseArgs(['--model', 'kev-4b', '--port', '9000', '--dir', 'somewhere', '--cuda'])).toEqual({
      ok: true,
      value: { model: 'kev-4b', port: 9000, dir: 'somewhere', cuda: true, help: false },
    })
  })

  it('accepts -h/--help', () => {
    expect(parseArgs(['--help'])).toEqual({ ok: true, value: { model: 'kev-0.8b', port: 8009, dir: '.local/kev', cuda: false, help: true } })
  })
...
describe('local-kev planCommands', () => {
  it('clones, syncs and serves when the target directory does not exist yet', () => {
    const steps = planCommands({ platform: 'win32', dir: '.local/kev', model: 'kev-0.8b', port: 8009, cuda: false, repoExists: false })
    ...
    expect(steps[2]).toMatchObject({
      command: 'uv',
      args: ['run', '--extra', 'serve', 'python', '-m', 'kev.serve', '--run', 'jaredpalmer/kev-0.8b', '--port', '8009'],
      cwd: '.local/kev',
    })
  })

  it('skips the clone step when the target directory already has a checkout', () => {
    const steps = planCommands({ platform: 'win32', dir: '.local/kev', model: 'kev-0.8b', port: 8009, cuda: false, repoExists: true })
    expect(steps.map((s) => s.id)).toEqual(['sync', 'serve'])
  })

  it('inserts the CUDA step before serve when --cuda is set, on Windows and Linux', () => {
    const steps = planCommands({ platform: 'linux', dir: 'kev', model: 'kev-9b', port: 8009, cuda: true, repoExists: true })
    expect(steps.map((s) => s.id)).toEqual(['sync', 'cuda', 'serve'])
    expect(steps[1].args.join(' ')).toBe(`pip install --python .venv torch torchvision --index-url ${CUDA_TORCH_INDEX_URL}`)
    expect(steps[1].cwd).toBe('kev')
  })

  it('omits the CUDA step on macOS even when --cuda is set', () => {
    const steps = planCommands({ platform: 'darwin', dir: 'kev', model: 'kev-0.8b', port: 8009, cuda: true, repoExists: true })
    expect(steps.map((s) => s.id)).toEqual(['sync', 'serve'])
  })

  it('reflects a custom model and port in the serve step', () => {
    const steps = planCommands({ platform: 'win32', dir: '.local/kev', model: 'kev-4b', port: 9100, cuda: false, repoExists: true })
    const serve = steps.find((s) => s.id === 'serve')!
    expect(serve.args).toEqual(['run', '--extra', 'serve', 'python', '-m', 'kev.serve', '--run', 'jaredpalmer/kev-4b', '--port', '9100'])
  })
})
