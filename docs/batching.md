...
- **Invalid text.** Every trim cuts on a code point boundary, and each state is sanitized before it is sent: lone surrogates become U+FFFD and C0 control characters other than tab and newline are dropped. If Jev still answers 400 `api_usage_error` "Request contains invalid Unicode text.", the request is retried once with stricter sanitizing (it also drops DEL, C1 controls, the BOM and noncharacters). If that fails too, the batch is split like a validation error, so only the offending issue fails.
