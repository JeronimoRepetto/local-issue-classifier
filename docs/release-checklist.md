...
- [ ] No personal data. Fixtures use synthetic users and repos (`acme/widgets`, `@jdoe`). No real e-mail addresses. No machine paths such as `C:\Users\…`.
  **Not ticked.** Every commit's author metadata (`git log`, not file content) carries the committer's real personal e-mail address — this predates Task 15, is outside what a file-content hygiene scanner can catch, and rewriting git history is a destructive operation out of this task's authorized scope. File content itself is clean (hygiene checker + a manual full-history content scan both pass — the address only shows up in commit metadata, never in a tracked file). Needs the user's decision before the repo goes public: for example, using a GitHub-provided no-reply address for future commits, or rewriting history with a history-rewriting tool if the existing author address must not stay public.
